...
default_cfgs = {
    'as_base_patch4_window7_224': _cfg(
        file='/root/paddlejob/workspace/env_run/finetune/asmlp_base_patch4_shift5_224.pth'
    ),
}
...
class PSRP(nn.Module):
    def __init__(self, channel=768, reduction=16):
        ...
        self.down = nn.Conv2d(channel, reduction, 1, 1, bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.up_weight = nn.Conv2d(reduction, channel, 1, 1, bias=False)
        self.up_bias = nn.Conv2d(reduction, channel, 1, 1, bias=False)
...
class AxialShiftedBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.tuning_mode == 'psrp':
            ...
            x = x + self.drop_path(bias + (weight+1)*self.mlp(self.norm2(x)))
        else:
...
class AS_MLP(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, probing_mode='mp', tuning_mode=None, **kwargs):
        super().__init__()
        ...
        if probing_mode == 'mp':
            self.head = Moment_Probing_MLP(in_dim=self.num_features, num_classes=num_classes)
        else: 
...
if __name__ =='__main__':
    ...
    model = as_base_patch4_window7_224(probing_mode='mp', tuning_mode='psrp',)  
    y = model(x)
    print(y.shape)  