...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, weight_init='', probing_mode='mp', tuning_mode=None, **kwargs):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.probing_mode == 'mp':
            cls_token = self.avgpool(self.norm(x).transpose(1, 2))  # B C 1
            cls_token = torch.flatten(cls_token, 1)
            return self.head(cls_token, x)
        else:
            x = self.avgpool(self.norm(x).transpose(1, 2))  # B C 1
            x = torch.flatten(x, 1)
            x = self.head(x)
        return x
...
if __name__ =='__main__':
    ...
    model = swin_base_patch4_window7_224_in22k(probing_mode='mp', tuning_mode='psrp',)  
    y = model(x)
    print(y.shape)  